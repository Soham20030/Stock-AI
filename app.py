...
with col_right:
    with st.container(border=True):
        st.markdown('<div class="vesper-title">🤖 AI Forecast Engine</div>', unsafe_allow_html=True)
...
with bottom_left:
    with st.container(border=True):
        st.markdown('<div class="vesper-title">📊 Model Accuracy Comparison</div>', unsafe_allow_html=True)
...
with bottom_right:
    with st.container(border=True):
        st.markdown(f'<div class="vesper-title">📰 Market News & Sentiment — {selected_stock}</div>', unsafe_allow_html=True)
